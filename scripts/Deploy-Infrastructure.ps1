...
function New-TerraformVarsFile {
    ...
    try {
        ...
        $haloAuthMethod = if ($HaloAuthUrl) { "oauth" } else { "apikey" }
        $haloAuthUrlValue = if ($HaloAuthUrl) { $HaloAuthUrl } else { "" }
        $content = $content -replace '\$\{HaloAuthMethod\}', $haloAuthMethod
        $content = $content -replace '\$\{HaloAuthUrl\}', $haloAuthUrlValue
...
if ($Action -eq "destroy") {
    ...
    while ($destroyCount -lt $maxDestroyRetries -and -not $destroySuccess) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            # Remove numbered state backup files generated by retry attempts
            Get-ChildItem -Path . -Filter "terraform.tfstate.*.backup" | Remove-Item -Force
    ...
    if (-not $destroySuccess) {
        ...
        $deletedApimsJson = az rest --method get `
            ...
            2>$null
        $deletedApims = $null
        if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($deletedApimsJson)) {
            try { $deletedApims = $deletedApimsJson | ConvertFrom-Json } catch { $deletedApims = $null }
        }
        if ($deletedApims -and $deletedApims.value -and $deletedApims.value.Count -gt 0) {
        ...
        # Remove numbered state backup files generated by retry attempts and state rm calls
        Get-ChildItem -Path . -Filter "terraform.tfstate.*.backup" | Remove-Item -Force
    }
...
if ($Action -in @("init", "plan", "apply", "all", "validate")) {
    ...
    if (-not (New-TerraformVarsFile -SubscriptionId $subscriptionId -Location $Location -Environment $Environment -HaloBaseUrl $HaloBaseUrl -HaloAuthUrl $HaloAuthUrl -OutputPath ".")) {
